...
export default function OverlayAddressState(props: Props): React.ReactElement {
  const { countryCode, onPress, selectedOption, setVisible } = props
  const listRef = React.useRef<FlatList | null>(null)
  const { states } = useListStates(countryCode)
  const { themeStyle } = useTheme()
  const styles = getStyles(themeStyle)
  const index = states.findIndex((c) => c.Value === selectedOption)
  return (
    <Animated.View
      ...
      <Animated.View
        ...
        <View style={themeStyle.overlayTitleRow}>
          <Text style={themeStyle.overlayTitleText}>Select State</Text>
