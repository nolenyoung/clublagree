...
export default function ClassSpotHexagon(props: Props): React.ReactElement {
  const { available, onPress, selected, text } = props
  const [size, setSize] = React.useState(0)
  const { themeStyle } = useTheme()
  const styles = getStyles(themeStyle, size)
  const xPos = Math.sin(Math.PI / 6) * (size / 2)
  const yPos = Math.cos(Math.PI / 6) * (size / 2)
  return (
    <Pressable
      disabled={!available}
      onLayout={(event) => {
        ...
        <Svg height={size} width={size}>
          {size > 0 && (
            <Polygon
              ...
              points={`${xPos},0 ${size / 2 + xPos},0 ${size},${yPos} ${size / 2 + xPos},${yPos * 2} ${xPos},${yPos * 2} 0,${yPos}`}
              y={(size - yPos * 2) / 2}
            />
          )}
        </Svg>
